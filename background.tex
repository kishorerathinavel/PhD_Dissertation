...
Fig. 2.1 shows a schematic diagram of the human eye. 
...
Fig. 2.2 shows the distribution of rods and cones. 
...
Some depth cues that we are aware of depend on information from both eyes. 
...
In (Cutting and Vishton, 1995b), an experiment was done where two objects A and B are shown at different depths, say D1 and D2, and the user is then asked to make a forced choice as to which object is closer. 
...
And the results of the experiments are summarized by the graph shown in Fig. 2.4. 
The horizontal axis of Fig. 2.4 shows the average distance of the two objects, i.e., $\frac{D_1 + D_2}{2}$ and the vertical axis shows the depth contrast between the two objects, i.e., $\frac{2\left(D_1 + D_2\right)}{D_1 + D_2}$.
...
\paragraph{Other monocular depth cues} There are many more monocular depth cues, e.g., some recently studied ones such as chormablur (Cholewiak et al., 2017), intra-pupillary occlusions (Zannoli et al., 2016), and depth cues that have been known for a long time such as motion parallax, relative size, atmospheric perspective, etc. 
...
Of all the display technologies that seek to integrate the real-world and the virtual-world (Sec. 2.2), only optical see-through AR displays propose to do in a portable manner and with minimum encumberance to the real-world view.
...
\begin{enumerate}
    ...
    \item \textbf{High Resolution:} The human visual system is capable of viewing resolution as high as 60 cyclesper- degree. However, the human visual system has such a high resolution only for a narrow region on the retina called the fovea. Beyond this region, the resolution drops drastically and is very low in the peripheral field-of-view. This non-uniform resolution across the field-of-view provides an opportunity to provide high resolution imagery without having to build very high resolution display panels but poses a challenge to dynamically change the display as the eye looks in different directions.
    \item \textbf{Depth cues:} AR displays should provide depth cues similar to that available in the real world. The various depth cues that the human visual system uses are discussed in Sec. 2.1.2. 
    \item \textbf{Low Latency:} AR displays need to respond fast to the user’s head and eye movements by updating the imagery being displayed. It is estimated that even 10 milliseconds delay between head motion and display update can result in 5 centimeters of error for a virtual object situated at 2 meters away. See Appendix A.1 for the calculation behind these numbers.
\end{enumerate}
...
Later, in each technical chapter (Chapters 4 and 3), a detailed review of previous work is presented in context for the technology presented in that chapter.
...
\begin{enumerate}
    \item \emph{Varifocal Displays:} (Dunn et al., 2017; Aks¸it et al., 2017) Provides synthetic focus cues, requires to track accommodation-state of the eye, and has latency in moving the in-focus plane.
    \item \emph{Multifocal Displays:} (Akeley et al., 2004a; Narain et al., 2015) Few focal planes which leads to partly synthetic focal cues and reduced spatial resolution for content in-between the few focal planes.
    \item \emph{Light-field Displays:} (Maimone et al., 2014a) Poor spatial resolution, frame-rates, and narrow depth-range. 
    \item \emph{Holographic Displays:} (Maimone et al., 2017) Very small eyebox, narrow depth-range.
\end{enumerate}

\subsection{Occlusion}

\emph{Fixed-focus occlusion displays} (Kiyokawa et al., 2003) can preserve a high-quality of the see-through view, but present the occlusion mask at a fixed distance. 

\emph{Light-field occlusion displays} (Maimone and Fuchs, 2013) attempts to provide depth-dependent occlusion by presenting a 4D light field occlusion mask using stacked LCD layers placed out of focus in front of the eye, where the occluding patterns are calculated by light field factorization algorithms (Lanman et al., 2010; Wetzstein et al., 2012). 
